...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    # 哪些参数需要更新，哪些参数不需要，weight-decay (L2正则化)
    ...
    optimizer = BertAdam(optimizer_grouped_parameters,
                         ...
                         warmup=0.1,
                         t_total=len(train_iter) * config.num_epochs)#earning_rate衰减
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):#每次取出一个batch数，更新一次梯度
            ...
            if total_batch % 100 == 0:
                ...
                train_acc = metrics.accuracy_score(true, predic) #训练集准确率
                dev_acc, dev_loss = evaluate(config, model, dev_iter) #验证集准确率
...
def evaluate(config, model, data_iter, test=False):
    ...
    with torch.no_grad():
        for texts, labels in data_iter:
            ...
            predic = torch.max(outputs.data, 1)[1].cpu().numpy()#torch.max 返回两个值，一个每个样本最大分类类别的概率，一个是最大值对应的索引，参数1是对每行求最大值
